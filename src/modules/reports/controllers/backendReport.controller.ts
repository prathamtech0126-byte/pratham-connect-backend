...
import { getCachedBackendReport } from "../cache/reports.cache.service";
...
const canViewBackendReport = (role: Role): boolean =>
  role === "developer" ||
  (BACKEND_REPORT_ROLES as readonly string[]).includes(role);

export const getBackendReportController = async (
  req: Request,
  res: Response
) => {
  try {
    ...
    if (!canViewBackendReport(viewer.role)) {
      return res.status(403).json({ success: false, message: "Forbidden" });
    }

    const filterParam = (req.query.filter as string) || "monthly";
    if (
      !BACKEND_REPORT_FILTERS.includes(
        filterParam as (typeof BACKEND_REPORT_FILTERS)[number]
      )
    ) {
      return res.status(400).json({
        success: false,
        message: `Invalid filter. Must be one of: ${BACKEND_REPORT_FILTERS.join(", ")}`,
      });
    }
    ...
    const result = await getCachedBackendReport(viewer, {
      filter: filterParam as (typeof BACKEND_REPORT_FILTERS)[number],
      fromDate,
      toDate,
      branchCode,
    });
    ...
    const message =
      error instanceof Error ? error.message : "Failed to fetch backend report";
    console.error("getBackendReportController error:", error);
